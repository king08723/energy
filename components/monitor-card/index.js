// components/monitor-card/index.js
Component({
  properties: {
    icon: {
      type: String,
  ...
  methods: {
    onTap() {
      ...
      this.triggerEvent('tap', {
        ...
        value: this.data.safeValue || this.data.value,
        unit: this.data.unit
      });
    },
  ...
  observers: {
    'value, trend': function (value, trend) {
      ...
      const safeTrend = this.safeStringValue(trend, 'stable');
  ...
  lifetimes: {
    attached() {
      ...
      const safeTrend = this.safeStringValue(this.data.trend, 'stable');
